...
data_path = 'C:/Data/100STYLE'
out_path = 'C:/Dev/100style-retarget/fbx'

styles = [folder for folder in os.listdir(data_path) if '.' not in folder]

for style in styles:

    bvh_files = [f for f in os.listdir(data_path+'/'+style) if f.endswith('.bvh')]

    for file in bvh_files:
        
        bvh_file = os.path.join(data_path+'/'+style, file)
        fbx_file = os.path.join(out_path, file.replace('.bvh', '.fbx'))

        print('Creating %s' % fbx_file)
        
        FBApplication().FileOpen('C:/Dev/100style-retarget/100style_character.fbx')
        FBApplication().FileImport(bvh_file, True)
        
        options = FBFbxOptions(True)
        options.TakeSpan = FBTakeSpanOnLoad.kFBLeaveAsIs
        
        FBApplication().FileMerge('C:/Dev/100style-retarget/Geno.fbx', False, options)
